...
def run_once(label, fitness_func, gene_type, n_genes, low, high,
    ...
    # --- wykres konwergencji dla najlepszego runa --------------------
    ...
    plt.plot(-history, label="Best Value")  # zmiana: -history
    plt.plot(-avg_history, label="Mean Value")  # zmiana: -avg_history
    plt.plot(std_history, label="Std Value")
    plt.title(f"{label} – best run convergence")
    plt.xlabel("Generation")
    plt.ylabel("Function Value")  # zmiana: lepszy opis
    plt.legend()
    plt.grid(True)
    plt.tight_layout()
    plt.savefig(f"{prefix}_convergence.png", dpi=200)
    plt.close()
    # --- zapis dodatkowych informacji do JSON -------------------------
    info = {
        "label": label,
